...
path = f"/cluster/home/huiluo/mt/grid_{grid}/{scenario_year}_{weekday}_{day_start_ts.month:02d}_{day_start_ts.day:02d}_{monitor_hr}_controlled"
...
hv_bus = str(89)
# Controlled charging
charge = pd.read_csv(f"{data_path}/Added_up_charge_{scenario_year}_raw.csv",index_col=0)
charge.index = pd.to_datetime(charge['ts'])
day1_ch = pd.DataFrame(charge.loc[day_start_ts:day_start_ts+pd.Timedelta(hours=23)][hv_bus])
day2_ch = pd.DataFrame(charge.loc[nexus_day2:nexus_day2+pd.Timedelta(hours=23)][hv_bus])
day3_ch = pd.DataFrame(charge.loc[nexus_day3:nexus_day3+pd.Timedelta(hours=23)][hv_bus])
day4_ch = pd.DataFrame(charge.loc[nexus_day4:nexus_day4+pd.Timedelta(hours=23)][hv_bus])
# day5_ch = pd.DataFrame(charge.loc[nexus_day5:nexus_day5+pd.Timedelta(hours=23)][hv_bus])
# day6_ch = pd.DataFrame(charge.loc[nexus_day6:nexus_day6+pd.Timedelta(hours=23)][hv_bus])
# day7_ch = pd.DataFrame(charge.loc[nexus_day7:nexus_day7+pd.Timedelta(hours=23)][hv_bus])
charge = pd.concat([day1_ch,day2_ch,day3_ch,day4_ch])#day5_ch,day6_ch,day7_ch])


discharge = pd.read_csv(f"{data_path}/EVBatt_power_hourly_{scenario_year}_discharge_mapped.csv",index_col=0)
discharge.index = pd.to_datetime(discharge.index)
day1_dis = pd.DataFrame(discharge.loc[day_start_ts:day_start_ts+pd.Timedelta(hours=23)][hv_bus])
day2_dis = pd.DataFrame(discharge.loc[nexus_day2:nexus_day2+pd.Timedelta(hours=23)][hv_bus])
day3_dis= pd.DataFrame(discharge.loc[nexus_day3:nexus_day3+pd.Timedelta(hours=23)][hv_bus])
day4_dis = pd.DataFrame(discharge.loc[nexus_day4:nexus_day4+pd.Timedelta(hours=23)][hv_bus])
# day5_dis = pd.DataFrame(discharge.loc[nexus_day5:nexus_day5+pd.Timedelta(hours=23)][hv_bus])
# day6_dis = pd.DataFrame(discharge.loc[nexus_day6:nexus_day6+pd.Timedelta(hours=23)][hv_bus])
# day7_dis = pd.DataFrame(discharge.loc[nexus_day7:nexus_day7+pd.Timedelta(hours=23)][hv_bus])
discharge = pd.concat([day1_dis,day2_dis,day3_dis,day4_dis])#,day5_dis,day6_dis,day7_dis])
...
def shape_matching(i,path):
    ...
    ## Vars
    m.SOE = pyo.Var(m.EVBAT,m.T,within=pyo.Reals,initialize=soe_init_rule)
    ...
    def next_trip_rule(m,park,person,arr_idx,end_idx,t):
        ...
        if end_idx==t and t<monitor_hr-1 and (end_idx!=arr_idx):
            return m.SOE[person,t]+sum((m.CHARGE_P[park,person,arr_idx,end_idx,t]-m.DISCHARGE_P[park,person,arr_idx,end_idx,t])*m.PARKHR[park,person,arr_idx,end_idx,t] for park,p,arr_idx,end_idx in m.PARK if p==person)>=m.NEXT_E[park,person,arr_idx,end_idx]
        else:
            return pyo.Constraint.Skip  
    ...
    def participate_rule(m,t):
        ...
        """
        return sum(m.FLEX[park,person,arr_idx,end_idx,t] for park,person,arr_idx,end_idx in m.PARK)<=0.27*m.PlugInCnt[t] + m.BUFFER[t]
    ...
    def hourly_discharge_power_sum_rule(m,t):
        return sum(m.DISCHARGE_P[park,person,arr_idx,end_idx,t] for park,person,arr_idx,end_idx in m.PARK)
    m.HourlyDischargedPowerSum = pyo.Expression(m.T,rule=hourly_discharge_power_sum_rule)

    ###########################
    # Objective 
    def objective_rule(m):
        return 1000*(sum((m.CHARGE_TARGET[t]-m.HourlyChargedPowerSum[t])**2 for t in m.T) + sum((m.DISCHARGE_TARGET[t]-m.HourlyDischargedPowerSum[t])**2 for t in m.T))+sum(m.BUFFER[t] for t in m.T)+sum(m.SOE_upper_slack[person,t]+m.SOE_lower_slack[person,t] for person in m.EVBAT for t in m.T)+sum(m.POWER_SHIFT_ABS[park,person,arr_idx,end_idx,t] for park,person,arr_idx,end_idx in m.PARK for t in m.T)
